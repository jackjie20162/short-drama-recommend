...
func New(c Config) *Client {
	return &Client{address:strings.TrimRight(c.Address,"/"),index:c.Index,http:&http.Client{Timeout:1500*time.Millisecond}}
}
...
func (c *Client) Search(ctx context.Context, req model.Query, size int) ([]model.Drama,error) {
	...
	body,err:=json.Marshal(FunctionScoreQuery(req,size)); if err!=nil{return nil,err}
	httpReq,err:=http.NewRequestWithContext(ctx,http.MethodPost,c.address+"/"+c.index+"/_search",bytes.NewReader(body)); if err!=nil{return nil,err}
	httpReq.Header.Set("Content-Type","application/json")
	resp,err:=c.http.Do(httpReq); if err!=nil{return nil,err}; defer resp.Body.Close()
	if resp.StatusCode<200||resp.StatusCode>=300 { b,_:=io.ReadAll(io.LimitReader(resp.Body,2048)); return nil,fmt.Errorf("elasticsearch status=%d body=%s",resp.StatusCode,strings.TrimSpace(string(b))) }
	var result struct{ Hits struct{ Hits []struct{ ID string `json:"_id"`; Source model.Drama `json:"_source"` } `json:"hits"` } `json:"hits"` }
	if err:=json.NewDecoder(resp.Body).Decode(&result);err!=nil{return nil,err}
	items:=make([]model.Drama,0,len(result.Hits.Hits))
	for _,hit:=range result.Hits.Hits { d:=hit.Source; if d.ID==0&&hit.ID!="" {if id,e:=strconv.ParseInt(hit.ID,10,64);e==nil{d.ID=id}}; items=append(items,d) }
	return items,nil
}
