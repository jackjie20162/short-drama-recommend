...
func main() {
	...
	seeds:=model.Seeds()
	ranker:=rank.New(c.Rank)
	esClient:=es.New(es.Config{Address:c.ES.Address,Index:c.ES.Index})
	svc:=logic.NewFeedLogic(seeds,esClient,ranker)
	server:=rest.MustNewServer(c.RestConf)
	defer server.Stop()
	handler.Register(server,svc)
	fmt.Printf("rec-api listening on %s\n",c.Host)
	server.Start()
}
