...
func(r *MySQLDramaAdminRepository)List(ctx context.Context,keyword,country,language string,status *int8,limit,offset int)([]*model.Drama,int64,error){
 ...
 rows,err:=r.db.QueryContext(ctx,"SELECT id,title,description,cover,country,language,total_episodes,is_paid,price_cents,currency,status FROM dramas"+where+" ORDER BY id DESC LIMIT ? OFFSET ?",append(args,limit,offset)...);if err!=nil{return nil,0,err};defer rows.Close()
 var out []*model.Drama
 for rows.Next(){d:=&model.Drama{};if err:=rows.Scan(&d.ID,&d.Title,&d.Description,&d.Cover,&d.Country,&d.Language,&d.TotalEpisodes,&d.IsPaid,&d.PriceCents,&d.Currency,&d.Status);err!=nil{return nil,0,err};out=append(out,d)}
