...
func (s *PaymentServer) createStripe(ctx context.Context,cents int64,currency,orderNo string)(string,string,error){
 key:=s.setting("stripe_secret_key",s.svcCtx.Config.Stripe.SecretKey);if key==""{return "","",errors.New("STRIPE_SECRET_KEY is not configured")}
...
func (s *PaymentServer) CapturePayment(ctx context.Context,r *pb.CapturePaymentRequest)(*pb.CapturePaymentResponse,error){
 ...
 if r.GetOrderId()>0 { if err:=s.svcCtx.DB.QueryRowContext(ctx,"SELECT id,provider,provider_order_id,status FROM orders WHERE id=? AND user_id=?",r.GetOrderId(),r.GetUserId()).Scan(&orderID,&provider,&providerID,&status);err!=nil{return nil,err} } else { if r.GetProviderOrderId()=="" { return nil,errors.New("order_id or provider_order_id is required") }; if err:=s.svcCtx.DB.QueryRowContext(ctx,"SELECT id,provider,provider_order_id,status FROM orders WHERE provider_order_id=? AND user_id=?",r.GetProviderOrderId(),r.GetUserId()).Scan(&orderID,&provider,&providerID,&status);err!=nil{return nil,err} }
...
if r.GetUserId()>0 {
 ...
 if err=s.svcCtx.DB.QueryRowContext(ctx,"SELECT COUNT(*) FROM orders").Scan(&total);err!=nil{return nil,err}
}
var rows *sql.Rows
if r.GetUserId()>0 {
 rows,err=s.svcCtx.DB.QueryContext(ctx,"SELECT id,user_id,drama_id,order_no,provider,provider_order_id,amount_cents,currency,status FROM orders WHERE user_id=? ORDER BY id DESC LIMIT ? OFFSET ?",r.GetUserId(),size,(page-1)*size)
} else {
 rows,err=s.svcCtx.DB.QueryContext(ctx,"SELECT id,user_id,drama_id,order_no,provider,provider_order_id,amount_cents,currency,status FROM orders ORDER BY id DESC LIMIT ? OFFSET ?",size,(page-1)*size)
}
if err!=nil{return nil,err}if err!=nil{return nil,err};defer rows.Close()
